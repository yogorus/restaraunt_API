...
async def patch_dish(client: AsyncClient, create_menu, create_submenu, create_dish):
    ...
    response = await client.patch(
        f"/menus/{create_menu['id']}/submenus/{create_submenu['id']}/dishes/{create_dish['id']}",
        json=sent_json,
    )
    response_json = response.json()
...
async def delete_dish(client: AsyncClient, create_menu, create_submenu, patch_dish):
    response = await client.delete(
        f"/menus/{create_menu['id']}/submenus/{create_submenu['id']}/dishes/{patch_dish['id']}",
    )
    assert response.status_code == 200
...
async def test_dish_by_id_after_delete(
    ...
    response = await client.get(
        f"/menus/{create_menu['id']}/submenus/{create_submenu['id']}/dishes/{delete_dish['id']}"
    )
...
async def test_submenu_list_after_submenu_delete(
    ...
    response = await client.get(f"{base_url}/menus/{create_menu['id']}/submenus/")
    assert response.status_code == 200
    assert response.json() == []


async def test_menu_list_after_menu_delete(client: AsyncClient, create_menu):
    response = await client.delete(f"{base_url}/menus/{create_menu['id']}")
    assert response.status_code == 200
